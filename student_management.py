...
def get_data(event):
    current = student_table.item(student_table.focus())#returns stored table row which is focused
    row = current["values"] #focussed row stored in current as a dictionary here dictionary is used  clear_all()
...
try:
    conn = sqlite3.connect('student_database.db')
    c = conn.cursor()#creates cursor used later on
    ...
    conn.commit()#to execute as c.execute wont do anything permt so this will do it forever
    conn.close()
